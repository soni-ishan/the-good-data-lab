...
export default async function ProjectArticlePage({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#0f0b08] text-stone-100">
      ...
      {/* ── Header strip ───────────────────────────────────────────── */}
      <div className="border-b border-amber-200/10 px-6 py-10 sm:px-8 lg:px-10">
        <div className="mx-auto w-full max-w-4xl">
          <Link
            href="/#projects"
            className="inline-flex items-center gap-2 text-sm font-medium text-stone-400 transition hover:text-amber-200"
          >
            ...
            <div className="mt-7 flex flex-wrap items-center gap-3">
              {project.links.code ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-full bg-amber-300/10 px-4 py-2 text-sm font-medium text-amber-200 transition hover:bg-amber-300/20"
                >
      ...
      <article className="mx-auto w-full max-w-4xl px-6 pb-24 pt-16 sm:px-8 lg:px-10">
          ...
          {project.article.sections.map((section, i) => (
            <section key={section.heading}>
              ...
              {i < project.article.sections.length - 1 && (
                <hr className="mt-16 border-amber-200/10" />
              )}
            </section>
          ))}
        </div>
        ...
        {/* ── Outcomes & Next Steps ──────────────────────────────────── */}
        <hr className="my-16 border-amber-200/10" />
        <div className="grid gap-12 sm:grid-cols-2">
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-300/70">Outcomes</h2>
          ...
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-300/70">Next Steps</h2>
            <ul className="mt-5 space-y-4">
        ...
        {/* ── Contributors ───────────────────────────────────────────── */}
        <hr className="my-16 border-amber-200/10" />
        <section className="rounded-2xl border border-amber-200/15 bg-[#1a130d]/80 p-6 sm:p-8">
          <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-amber-300/70">Contributors</h2>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {projectContributors.map((contributor) => (
              <div key={contributor.id} className="rounded-xl border border-amber-200/15 bg-black/10 p-4">
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-amber-300/20 text-sm font-bold text-amber-100">
                    {contributor.avatarInitials}
                  </div>
                  <div>
                    <h3 className="font-semibold text-stone-100">{contributor.name}</h3>
                    <p className="text-sm text-amber-200">{contributor.role}</p>
                  </div>
                </div>
                <p className="mt-3 text-sm leading-relaxed text-stone-300">{contributor.bio}</p>
                <div className="mt-3 flex items-center gap-4">
                  {contributor.links.linkedin ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center gap-1.5 text-sm text-stone-300 transition hover:text-amber-200"
                    >
